...
class App:
    def __init__(self, h, l, titre, fps):
        ...
        px.load("Kenji_Battle_Ressources.pyxres")
        
        stockage = px.user_data_dir("pyxel", "Kenji_Battle")
        données_c = stockage + r"Données_communes"
        if not os.path.exists(données_c):
            os.makedirs(données_c)
            
        données_p = stockage + r"Données"
        if not os.path.exists(données_p):
            os.makedirs(données_p)
        
        global last_pseudo
        last_pseudo = stockage + r"Données_communes\Last_pseudo.sq3"
        global scores
        scores = stockage + r"Données_communes\Scores.sq3"
        
        self.données = Base_donées()
        
        if not os.path.isfile(last_pseudo):
            self.données.hard_init()
                ...
                for ennemi_s in self.ennemis_spawn:
                    ...
                    for ennemi in ennemi_s.ennemis:                        
                        ...
                        for slash in self.perso.slashs:
                            b = slash
                            if numero_ennemi not in self.ennemis_t:
                                if slash.chek_touche(ennemi.x, ennemi.width):
                                    ennemi.touché(2)
                                    self.ennemis_t.append(numero_ennemi)
                                    if slash.suppression():
                                        self.perso.slashs.remove(b)
                            else:
                                self.ennemis_t.remove(numero_ennemi)
...
class Paramètres:
    ...
    def refresh_données(self):
        ...
        if len(liste_params) == 0:
            self.scale = 2
            self.difficultée = ("Moyen", 9, "Un mode juste normal/-3 ennemis max/-3 types/-4 vies/x1 suchis")
            a = 1
        else:
            for taille, difficultée in liste_params:
                if (taille, difficultée) == liste_params[-1]:
                    self.scale = taille
                    for difficultée_list, c, b in self.difficultée_list:
                        if difficultée == difficultée_list:
                            a = self.difficultée_list.index((difficultée_list, c, b))
                            self.difficultée = self.difficultée_list[a]
                            
        self.boutons = [
                        Bouton(px.width - 46, 15, 1, "Croix"),
                        Bouton(400, 53, 2, "Bouton"),
    ...
    def draw(self):
        ...
        draw_texte(280, 53, "3.Changer le pseudo/(= changer de sauvegarde)", 1, 7, True)
        for bouton in self.boutons:
            bouton.draw()
...
class Boutique:
    ...
    def refresh_données(self):
        ...
        self.suchis = self.données.recup_generale()
    ...
    def refresh_objets(self):
        ...
        self.upgrades = []
        self.objets_achetés = []
        for u_type, prix, valeur, paramètre, acheté in liste:
            if acheté == 0:
                        ...
                        if self.upgrades[i].bouton.état():
                            if self.suchis >= self.upgrades[i].prix:
                                ...
                                if self.upgrades[i].nom_variable != "None":
                                    ...
                                    for a in range(len(self.list_paramètres)):
                                        if self.upgrades[i].type == self.list_paramètres[i][0]:
                                            if a in self.increment:
                                                if self.upgrades[i].valeur >= self.list_paramètres[a][1]:
                                                    self.refresh_stats(a, self.upgrades[i].valeur)
                                            elif a in self.decrement:
...
class Bouton:
    def __init__(self, x, y, scale, b_type):
        self.scale = scale
        self.type = b_type
        
        self.liste_types = (
                            (None, "Play", 0, 32, 128, 64, 32),
                            (None, "Paramètres", 0, 64, 96, 32, 32),
                            (None, "Information", 32, 64, 96, 32, 32),
                            (None, "Croix", 80, 32, 48, 16, 16),
                            (self.bouton, "Bouton", 112, 16, 24, 12, 8),
                            (None, "Boutique", 128, 128, 144, 16, 16),
                            (None, "Vitesse", 64, 32, 48, 16, 16),
                            (None, "Ultimate", 176, 64, 80, 16, 16),
                            (None, "Amélioration/de/l'ulti", 176, 64, 80, 16, 16),
                            (None, "Temps de/charge", 160, 0, 16, 16, 16),
                            (None, "Revenus/score", 176, 0, 16, 16, 16),
                            (None, "Revenus/suchis", 160, 32, 48, 16, 16),
                            (None, "Parade", 176, 32, 48, 16, 16),
                            (self.acheter, "Acheter", 144, 128, 144, 48, 16),
                            (None, "Score", 208, 0, 16, 16, 16),
                            (None, "Une 3eme/charge", 192, 64, 80, 16, 16)
                            )
        for _, types, _, _, _, width, height in self.liste_types:
            if b_type == types:
    ...
    def action(self):
        for b_type, nom, img_x, img_y1, img_y2, width, height in self.liste_types:
            if self.type == nom:
                self.width = width
                self.height = height
                self.img_x = img_x
    ...
    def draw(self):
        ...
        if self.type == "Boutique":
            px.pal(8, 16)
        px.blt(self.x, self.y, 0, self.img_x, self.img_y, self.width, self.height, 15, scale = self.scale)
        if self.texte != None:
...
class Base_donées:
    def hard_init(self):
        conn = sqlite3.connect(last_pseudo)
        cur = conn.cursor()
        cur.execute("create table Pseudos(pseudo text)")
        cur.execute("insert into Pseudos(pseudo) values('None')")
        conn.commit()
        cur.close()
        conn.close()
    ...
    def init(self, Pseudo):
        path = px.user_data_dir("pyxel", "Kenji_Battle") + fr"Données\{Pseudo}"
        
        if not os.path.exists(path):
            os.makedirs(path)
        
        global données
        données = path + r"\Données.sq3"
        if not os.path.isfile(données):
            conn = sqlite3.connect(données)
            cur = conn.cursor()
            
            cur.execute("create table Paramètres ( Taille integer, Difficultée text)")
            cur.execute("create table Générale (suchis integer)")
            cur.execute("create table Objets (type text, prix integer, valeur integer, paramètre text, acheté boolean default 0)")
            
            list_objets = (
                        "('Vitesse', 500, 1.25, null)",
                        "('Ultimate', 1000, 'True', null)",
                        "('Amélioration/de/lulti', 1500, 5, 'Rechargement')",
                        "('Temps de/charge', 600, 0.5, 'Delay 1ere/charge')",
                        "('Temps de/charge', 600, 1, 'Delay 2eme/charge')",
                        "('Revenus/suchis', 700, 1.10, null)",
                        "('Revenus/score', 700, 1.10, null)",
                        "('Parade', 400, 10, null)",
                        "('Une 3eme/charge', 600, 'True', null)"
                        )
            objets = ", "
            objets = objets.join(list_objets)
            
            if Pseudo == 'Ethan':
                cur.execute("insert into Générale(suchis) values(5000)")
            else:
                cur.execute("insert into Générale(suchis) values(0)")
            cur.execute(f'insert into Objets(type, prix, valeur, paramètre) values {objets}')
        ...
        conn = sqlite3.connect(last_pseudo)
        cur = conn.cursor()
        cur.execute(f'update Pseudos set pseudo = "{Pseudo}"')
        conn.commit()
        cur.close()
        conn.close()
    ...
    def chek_new(self):
        conn = sqlite3.connect(last_pseudo)
        cur = conn.cursor()
        cur.execute("select * from Pseudos")
        liste = list(cur)
        if liste[0][0] == 'None':
            cur.close()
            conn.close()
    ...
    def recup_objets(self):
        conn = sqlite3.connect(données)
        cur = conn.cursor()
        cur.execute("select type, prix, coalesce(valeur, 'None'), coalesce(paramètre, 'None'), acheté from Objets")
        v = list(cur)
        cur.close()
        conn.close()  
        return v
    ...
    def recup_paramètres(self):
        conn = sqlite3.connect(données)
        cur = conn.cursor()
        cur.execute("select * from Paramètres")
        v = list(cur)
        cur.close()
        conn.close() 
        return v
    ...
    def recup_score(self):
        conn = sqlite3.connect(scores)
        cur = conn.cursor()
        cur.execute("select * from Hscores")
        v = list(cur)
        cur.close()
        conn.close() 
        return v
    
    def recup_generale(self):
        conn = sqlite3.connect(données)
        cur = conn.cursor()
        cur.execute("select * from Générale")
        liste = list(cur)
        liste = liste[0][0]
        cur.close()
        conn.close() 
    ...
    def ajouter_suchis(self, suchis):
        conn = sqlite3.connect(données)
        cur = conn.cursor()
        cur.execute(f"update Générale set suchis = {suchis}")
        conn.commit()
        cur.close()
        conn.close() 
    ...
    def acheter_objet(self, nom, suchis, valeur):
        ...
        cur.execute(f"update Objets set acheté = 1 where valeur = {valeur} and type = '{nom}'")
        cur.execute(f"update Générale set suchis = {suchis}")
        conn.commit()
        cur.close()
        conn.close() 
    ...
    def sauvegarder_param(self, taille, difficultée):
        conn = sqlite3.connect(données)
        cur = conn.cursor()
        cur.execute(f"insert into Paramètres (Taille, Difficultée) values ({taille}, '{difficultée}')")
        conn.commit()
        cur.close()
        conn.close() 
    ...
    def ajouter_score(self, score, joueur):
        conn = sqlite3.connect(scores)
        cur = conn.cursor()
        cur.execute(f"insert into Hscores(Score, Joueur) values ({score}, '{joueur}')")
        conn.commit()
        cur.close()
        conn.close()
