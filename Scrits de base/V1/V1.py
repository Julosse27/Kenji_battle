...
class App:
    def __init__(self, h, l, titre, fps):
        ...
        global last_pseudo, scores, données_p, données_c
        
        données_c = px.user_data_dir("pyxel", "Kenji_battle v1") + r"Données_communes"
        if not os.path.exists(données_c):
            os.makedirs(données_c)
        
        last_pseudo = données_c + r"\Last_pseudo.sq3"
        scores = données_c + r"\Scores.sq3"
        
        données_p = px.user_data_dir("pyxel", "Kenji_battle v1") + r"Données"
...
class Base_donées:
    ...
    def reinitialisation(self):
        shutil.rmtree(données_p)
        os.makedirs(données_p)
        os.remove(last_pseudo)
        os.remove(scores)        
        self.hard_init()
        
    def init(self, Pseudo):
        path = données_p + fr"\{Pseudo}"
        ...
        if not os.path.isfile(données):
            ...
            cur.execute("create table Objets (type text, prix integer, valeur integer, paramètre text, acheté boolean default 0, deblocage_nom text, deblocage_valeur integer)")
            
            list_objets = (
                        "('Parade', 400, 10, null, null, null)",
                        "('Vitesse', 500, 1.20, null, null, null)",
                        "('Temps de/charge', 600, 0.5, 'Delay 1ere/charge', null, null)",
                        "('Revenus/suchis', 700, 1.10, null, null, null)",
                        "('Une 3eme/charge', 750, 'True', null, null, null)",
                        "('Revenus/score', 800, 1.10, null, null, null)",
                        "('Temps de/charge', 900, 1, 'Delay 2eme/charge', null, null)",
                        "('Parade', 1000, 20, null, 'Parade', 10)",
                        "('Ultimate', 1000, 'True', null, null, null)",
            ...
            cur.execute(f'insert into Objets(type, prix, valeur, paramètre, deblocage_nom, deblocage_valeur) values {objets}')
    ...
    def recup_objets(self):
        conn = sqlite3.connect(données)
        cur = conn.cursor()
        cur.execute("select prix, type, valeur, paramètre, acheté from Objets where deblocage_nom is null")
        v = list(cur)
        cur.close()
        conn.close()
        return v
    
    def recup_require(self):
        conn = sqlite3.connect(données)
        cur = conn.cursor()
        cur.execute("select prix, type, valeur, paramètre, acheté, deblocage_nom, deblocage_valeur from Objets where deblocage_nom not null")
        liste_require = list(cur)
        cur.execute("select * from Objets")
        liste_requirement = []
        for i in range(len(liste_require)):
            if liste_require[i][6] == 'True':
                cur.execute(f"select type, valeur, acheté from Objets where type = '{liste_require[i][5]}'")
            else:
                cur.execute(f"select type, valeur, acheté from Objets where type = '{liste_require[i][5]}' and valeur = {liste_require[i][6]}")
            liste = list(cur)
